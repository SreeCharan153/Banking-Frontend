...
# In dev (localhost, http) -> secure=False so cookies still work
# In prod (Render, https)  -> secure=True (required for SameSite=None)
COOKIE_SECURE = ENV == "prod"
COOKIE_SAMESITE = "none"  # required for cross-site (Vercel -> Render)
...
def get_client() -> Client:
    ...
    DO NOT pass your app JWT here. Supabase expects its own auth, not your atm_token.
    """
...
def get_service_client() -> Client:
    """
    Service role client for privileged operations (logging, admin stuff).
    """
...
async def attach_supabase(request: Request, call_next):
    # No more poisoning Supabase with atm_token.
    client = get_client()
...
def get_current_user(request: Request) -> Dict[str, str]:
    ...
    client: Client = getattr(request.state, "supabase", None) or get_client()
    res = (
        client.table("users")
        .select("uid, role")
        .eq("uid", user_id)
    ...
    # Opportunistic refresh if access about to expire
    exp = claims.get("exp")
...
def create_user(data: CreateUserRequest, _: Dict = Depends(require_roles("admin"))):
    ...
    client = get_client()
    # Assuming "uid" is your primary identifier used everywhere else
    res = (
        client.table("users")
        .select("uid")
...
def deposit(
    ...
    _: Dict = Depends(require_roles("admin", "teller", "custmor")),
):
